describe('ticketazo', () => {
    ...
    it('Limpiar Localidad', () => {
        ...
        //Localidad
        cy.get('[data-slot="value"]').eq(2).should('be.visible').click()
        cy.get('[data-slot="listbox"]').should('be.visible')
        cy.contains('[role="option"]', 'Rawson').should('be.visible').click()

        cy.contains('Limpiar filtros').click()   
        cy.get('[data-slot="value"]').eq(1).should('have.value', '')
         cy.get('[data-slot="value"]').eq(2).should('not.exist')
    });
})