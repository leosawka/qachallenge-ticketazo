describe('ticketazo', () => {
    ...
    it('Limpiar Categoria', () => {
        cy.get('[data-slot="innerWrapper"]').eq(0).click({force: true})
        cy.get('[data-slot="content"]').should('be.visible')
        cy.contains('[role="option"]', 'Fiesta').should('be.visible').click()
        cy.contains('Limpiar filtros').click() 
        cy.get('[data-slot="value"]').eq(0).should('have.value', '')  
    });
})